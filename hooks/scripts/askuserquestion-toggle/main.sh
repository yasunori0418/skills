...
# マーカー（/tmp/claude-no-askuserquestion.<session_id>）を作成/削除する。
#   #aq-off → 無効化（マーカー作成）
#   #aq-on  → 再有効化（マーカー削除）
# セッション単位なので、同一 PC・同一プロジェクトの並行セッションには影響しない。
# session_id は Bash 環境変数には無く、hook の stdin JSON で渡るためここで解決する。
# 両方のトークンが含まれる場合は #aq-off を優先（無効化を安全側とみなす）。
#
# Claude Code の UserPromptSubmit hook として stdin で JSON を受け取る。トークンを
