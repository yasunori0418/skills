...
境界を正当に広げる手順:
  1. オーケストレーション配下のレーンなら、lane-ops の report.sh で親セッションへ「境界の不足」を報告して指示を待つ（拡張は親だけが lane-ops の widen_boundary.sh で行う）
  2. 親のいない単独セッションなら、なぜ境界の外に手を入れる必要があるのかをユーザーへ説明し、ユーザーの判断で境界を広げてもらう
...
対処（いずれか）:
  ...
  2. この変更が本当に現在のタスクに必要なら: オーケストレーション配下のレーンなら lane-ops の report.sh で親セッションへ報告して指示を待つ（拡張は親だけが lane-ops の widen_boundary.sh で行う）。親のいない単独セッションなら理由をユーザーへ説明し、ユーザーの判断で境界を広げてもらう。AI が自分で境界ファイルを書き換えることはできない
