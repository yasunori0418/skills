#!/usr/bin/env bash
# Verifies teammate-leak-guard (Stop hook: background_tasks の残留を additionalContext で通知):
#   - subagent / teammate が残っていれば体数と一覧を返す
...
# additionalContext を取り出す（沈黙時は空文字）
context() { # json
    printf '%s' "$1" | "$GUARD" | jq -r '.hookSpecificOutput.additionalContext // empty'
}
# additionalContext の 1 行目（体数のサマリ行）だけを取り出す
summary() { # json
    context "$1" | head -1
}
...
check "broken-json" "" "$(printf 'not json' | "$GUARD" | jq -r '.hookSpecificOutput.additionalContext // empty' 2>/dev/null || true)"
