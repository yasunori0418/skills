...
1 マーケットプレイス(`marketplace.json`)に **7 つのプラグイン**を列挙している。
...
| `claude-skills`      | `./skills/claude`   | Claude Code 固有: latency-triage / response-format / session-insights / tmp-output |
...
> それ以外の skill 非依存 hook は `yasunori0418-hooks` にまとめ、独立して install できる。
