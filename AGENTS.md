...
| 配布・パッケージング | Claude Code plugin | per-category `skills/<category>/.claude-plugin/plugin.json` + `.claude-plugin/marketplace.json`（6 プラグイン） |
| Codex 連携 | 本リポジトリの慣習 | per-skill `agents/openai.yaml` |
| claude-code サブエージェント（任意） | Claude Code plugin | per-skill `agents/<name>.md` |
| 運用 hook | Claude Code plugin | 汎用: `hooks/hooks.json`（yasunori0418-hooks）/ skill 連動: `skills/<category>/hooks/hooks.json` |

## プラグイン構成（カテゴリ別 N plugins）

1 マーケットプレイス（`.claude-plugin/marketplace.json`）に 6 プラグインを列挙し、
利用者はカテゴリ単位で install できる。

- **カテゴリプラグイン**（5 個）: `skills/<category>/` が各プラグイン root。
...
- **`yasunori0418-hooks` プラグイン**: root=リポジトリ直下（`source: "./"`）。
  リポジトリ直下の `.claude-plugin/plugin.json` がこのプラグインの manifest で、
  `hooks/hooks.json` を規約検出する。skill は含まない。
- **hook の所属**: skill に依存しない guard/通知 hook は `yasunori0418-hooks` に置く。
  skill とペアで機能する hook（例 `git-guard` は rebase-flow/reset-flow の arm marker と
  ペア）は、その skill のカテゴリプラグインに `skills/<category>/hooks/` として同居させる。
...
- plugin hooks は `hooks.json` に定義し、実体は `<name>/main.sh`
  （テストは `<name>/tests/*.test.sh`、`checks.hooks` が実行）。汎用 hook は `hooks/` 直下、
  skill 連動 hook は `skills/<category>/hooks/` 配下。スクリプト参照は各プラグイン root を
...
- hook スクリプトのユニットテストは `checks.hooks`（`nix flake check`）で実行する。
  ...
  （汎用 hook の `hooks/<name>/tests/` と skill 連動 hook の
