...
| 運用 hook | Claude Code plugin | `hooks/hooks.json` + `hooks/<name>/` |
...
- インフラ（`flake.nix` / `dev/` / `pkgs/` / `schema/` / `scripts/` / `hooks/` / `.github/` /
  `.claude-plugin/`）はリポジトリ直下。スキルのカテゴリディレクトリと混在する。
- plugin hooks は `hooks/hooks.json` に定義し、実体は `hooks/<name>/main.sh`
  （テストは `hooks/<name>/tests/*.test.sh`、`checks.hooks` が実行）。
  スクリプト参照は `${CLAUDE_PLUGIN_ROOT}` 起点で書く。
...
1. `<category>/<skill-name>/` ディレクトリを新規作成し `SKILL.md` を置く（雛形ディレクトリは
...
6. **新しいカテゴリを追加したら `.claude-plugin/plugin.json` の `skills` 配列に必ず追記する**
   （例: `"skills": ["./git", "./aws"]`）。カテゴリ別配置のため Claude Code の
...
- hook スクリプトは `hooks/<name>/tests/*.test.sh` のユニットテストを
  `checks.hooks`（`nix flake check`）で実行して検証する。
...
- `flake.nix` は `schema/`・`scripts/`・`hooks/` を参照するため、それらを先行コミットして
