...
| 配布・パッケージング | Claude Code plugin | per-category `skills/<category>/.claude-plugin/plugin.json` + `.claude-plugin/marketplace.json`（10 プラグイン） |
...
1 マーケットプレイス（`.claude-plugin/marketplace.json`）に 10 プラグインを列挙し、
利用者はカテゴリ単位・hook 単位で install できる。

- **カテゴリプラグイン**（6 個）: `skills/<category>/` が各プラグイン root。
