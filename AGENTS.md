...
- plugin hooks は `hooks/hooks.json` に定義し、実体は `hooks/scripts/<name>/main.sh`
  （テストは `hooks/scripts/<name>/tests/*.test.sh`、`checks.hooks` が実行）。
...
- hook スクリプトは `hooks/scripts/<name>/tests/*.test.sh` のユニットテストを
