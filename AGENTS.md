...
（カテゴリ 8 + hook 8 + バンドル 1）を列挙し、利用者はカテゴリ単位・hook 単位・
バンドル単位で install できる。
...
- **hook プラグイン**（8 個）: skill 非依存の guard/通知 hook を **hook 単位**で
  ...
  `yasunori0418-broad-scan-guard-hooks` / `yasunori0418-noclobber-guard-hooks` /
  `yasunori0418-notify-stop-hooks` / `yasunori0418-task-boundary-hooks` /
  `yasunori0418-teammate-leak-guard-hooks`）。root 直下に
