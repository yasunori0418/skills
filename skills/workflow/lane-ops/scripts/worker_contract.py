...
TEMPFILES_CLAUSE = (
    ...
    "自分で名前を決めるファイルは、衝突を避けるために `mktemp -d` で一意な名前を使うか、"
    "タスク ID を付ける。"
    "`tmp_claude/` などリポジトリ内の gitignore されたディレクトリは worktree 間で"
    "symlink 共有されていることがあり（scratchpad と違い他レーンと実体を共有する）、"
    "スキルが既定名を決めているファイルは他レーンのものかもしれない。"
    "見つけても消さず上書きしない。"
