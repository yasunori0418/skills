...
確認ダイアログになって停止するため、条項は「消さない」で統一する。条項に含める 4 点:
...
- **`mktemp -d` で一意な名前を使う** — 消さない運用は名前の衝突と表裏なので、
  固定名を避けて衝突自体を起こさない。スキル既定のファイル名にはタスク ID を付ける:
  gitignore されたディレクトリ（`tmp_claude/` 等）は worktree 作成フックにより
  primary への symlink になっていることがあり、全レーンが同じ実体を共有する。
  実運用で既定名の状態ファイルを別レーンが上書きする事故が起きた。既定名のファイルを
  見つけても他レーンのものとみなし、消さず上書きしない。
