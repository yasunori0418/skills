...
境界 / issue 参照 / コミット粒度 / 一時ファイルの扱い / 停止時の通知 /
報告（マイルストーン一覧を除く）/ サブエージェント委任 / サブエージェントの生存管理 /
進捗ナレーション。

**一時ファイルの扱い**はコミット粒度の直後に置く。レーンでは一時ファイルの削除が
確認ダイアログになって停止するため、条項は「消さない」で統一する。条項に含める 5 点:

- **scratchpad の一時ファイルは消さない** — 後始末のつもりの削除がレーンを止める。
  消さずに残すほうが安い（scratchpad はセッション専用で汚しても他へ波及しない）。
- **`mktemp -d` で一意な名前を使う** — 消さない運用は名前の衝突と表裏なので、
  固定名を避けて衝突自体を起こさない。この指示が掛かるのは**自分で名前を決めるファイル**で、
  一意名かタスク ID を付ける。
- **共有ディレクトリの既定名ファイルは消さず上書きしない** — gitignore された
  ディレクトリ（`tmp_claude/` 等）は worktree 作成フックにより primary への symlink に
  なっていることがあり、全レーンが同じ実体を共有する。実運用で既定名の状態ファイルを
  別レーンが上書きする事故が起きた。
  スキルが既定名を決めているファイルはワーカーの一存で改名できないため、条項は
  「他レーンのものとみなして触らない」までを求める。
- **上書きは `>|` で行う** — シェルは noclobber のため素の `>` は既存ファイルへ書けず、
  「消してから書く」動機になる。削除の代わりに上書きで済ませる。
- **実測検証は scratchpad に置いたコピーの上で行い、worktree のファイルを
  書き換えて戻す手順は禁止** — 戻し損ねが差分に混ざり、戻す操作自体も削除を誘発する。

報告条項には、コマンド名を含む報告に `report.sh --file <path>` を使う案内も入れる
（本文を引数へ載せると guard hook がコマンド名へ反応して報告自体が止まる）。案内は次の 3 点で
構成する: 短縮形の案内 / そのまま実行できる完全形と `--file` の先頭固定（末尾に置くと本文の
一部として扱われ、報告は成功したまま本文が壊れる）/ 本文ファイルは Write ツールで書くこと
（heredoc で書くとコマンド名が Bash 引数に載り直す）。
`parent` が空だと報告条項が「省略してよい」の文言へ差し替わるため、この案内も出ない。

**停止時の通知**は報告条項から独立させ、その直前に置く（implement で `parent` が空なら
報告と同様に出さない。maintain は `parent` 必須なので常に出る）。
