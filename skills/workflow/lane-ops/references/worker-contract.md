...
- **共有ディレクトリの既定名ファイルは消さず上書きしない** — gitignore された
  ...
  なっていることがあり、全レーンが同じ実体を共有する（scratchpad はセッション専用で
  共有されない）。実運用で既定名の状態ファイルを別レーンが上書きする事故が起きた。
...
（本文を引数へ載せると guard hook がコマンド名へ反応して報告自体が止まる）。本文ファイルは
Write ツールで書くことも併記する（heredoc で書くとコマンド名が Bash 引数に載り直す）。
