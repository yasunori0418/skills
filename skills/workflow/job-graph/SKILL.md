...
`mode: "maintain"`（Phase 4.5）では `PR` / `VERIFY` が出力されないため、上記のうち **PR 戦略と計画突合の基準だけを差し替える**（起動ウェーブとレーン割当・コミット計画・承認代行の宣言はそのまま含める）。差し替え先は「どのレビュー指摘へ対応するか」と「push は都度親が承認する」ことの明記（詳細は `references/maintain.md`）。
