...
- **`scripts/plan_orchestration.py`**: JSON spec を入力に、循環検出・base 解決・ウェーブ算出・レーン割当・ワーカープロンプトのファイル書き出し・wt/herdr コマンド列生成を行う:
  ...
  UV_PROJECT_ENVIRONMENT="$HOME/.cache/uv-venvs/job-graph" uv run --project "<SKILL>" \
    ...
    --prompt-dir "$PWD/tmp-agents/<job>/job-graph/prompts" --parent-name <親のエージェント名> tmp-agents/<job>/job-graph/spec.json
...
3. `plan_orchestration.py` を `--prompt-dir`（`tmp-agents/<job>/job-graph/prompts`。scratchpad は親交代で失効するので使わない。理由は `references/spec.md`）と `--parent-name`（上記の名前）付きで実行し、`ERROR` が出たら spec を直して再実行。
...
`tmp-agents/<job>/handoff.md` を生成し、状態が変わるたび（wave 完了・PR 作成・
