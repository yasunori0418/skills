...
- **`scripts/preflight.sh`**（read-only）: HERDR_ENV・ツール有無・既定ブランチ・`permissions.ask` の git 系ルール（レーンでは確認ダイアログになり blocked。親が lane-ops の承認代行の判定基準で捌くため `NOTE` 扱い）・未コミット変更・名前衝突を収集。`WARNING` を解消してから進む（`NOTE` は解消対象ではない）。
