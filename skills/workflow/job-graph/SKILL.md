...
herdr 上のトポロジ（workspace = 並列レーン / tab = レーン内の直列段）と各 ID の取り回しはスクリプトが `LANES` / `COMMANDS` として決定論的に算出する。手で決めない。
...
- 進捗確認: `wt list`
- 全レーンのマージ後: `/post-merge-cleanup` を案内する。herdr の workspace/tab は**自分が作ったものだけ**閉じる（`herdr workspace close`）
