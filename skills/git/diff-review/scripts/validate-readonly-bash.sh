...
# collect-diff.sh は本スクリプトと同じディレクトリに同梱されている(plugin 配布でもパスが成立する)
COLLECT_SH="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd -P)/collect-diff.sh"
...
for seg in "${SEGMENTS[@]}"; do
    ...
    # symlink 経由(plugin cache 等)の呼び出しでも一致するよう物理パスに正規化
    if command -v realpath >/dev/null 2>&1; then
        first_path="$(realpath -m "$first_path" 2>/dev/null || printf '%s' "$first_path")"
    fi
