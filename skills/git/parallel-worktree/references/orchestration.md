...
**手書きは不要**。`plan_orchestration.py` が spec の `prompt`（タスク本文）の後ろへ下記の標準セクションを自動で連結する。境界・TDD 順序・PR 前ゲート・コミット粒度・委任抑制・スコープ制約・ナレーション抑制が全 task に必ず載るので、spec の `prompt` には**タスク固有の内容と完了条件だけ**を書く。
...
- サブエージェント委任: 複数ファイル横断調査のような真に独立した大きな作業に限る。数回のツール呼び出しで済む作業は委任しない。
...
- 進捗ナレーション: 最初のツール呼び出し前に 1 文だけ宣言し、以降は重要な発見・方針転換のときのみ短く述べる（detached tmux で逐次読む人はいない）
```
...
- **委任抑制・スコープ制約・ナレーション抑制の 3 項**は、Opus 5 の傾向（サブエージェント委任に積極的・要求されていないステップを足してスコープを拡大しやすい・進捗ナレーションが増える）への対策。境界 glob + hook が止められるのは**ファイル単位の逸脱**だけで、境界**内**でのスコープ拡大とサブエージェント乱発は指示層でしか抑えられない。ナレーション抑制は detached tmux に読み手がいない状況でのトークン浪費対策。旧モデル（Opus 4.8 以前）へ同じ指示を与えても害は無い（委任・スコープ拡大が元々控えめなだけ）のでモデル分岐はしない
