...
- **委任抑制・スコープ制約・ナレーション抑制の 3 項**は、ワーカーが detached tmux で人の介入無しに長時間走ることへの対策。境界 glob + hook が止められるのは**ファイル単位の逸脱**だけで、境界**内**でのスコープ拡大（頼まれていない改善・追加作業）とサブエージェント乱発は指示層でしか抑えられない。ナレーション抑制は読み手のいない pane へ進捗宣言を書き続けるトークン浪費への対策
