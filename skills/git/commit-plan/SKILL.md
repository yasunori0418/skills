...
**計画を立てる作業すべてで、実装ステップと対になるコミット計画を含める**。Claude の plan モード（`ExitPlanMode` で提示する plan）に限らず、計画ドキュメント・並列作業のタスク分解（job-graph の spec・各エージェントへの指示文）など、**計画を成果物として書き出す場面すべて**が対象。実装ステップだけの計画で実装に入らない。
...
job-graph 等で作業を並列タスクへ分解するときは、コミット計画も**タスク（ブランチ／エージェント）単位**で立て、各タスクの指示文（spec の prompt 等）へ含める。タスク間で 1 つのコミット計画を共有しない（ブランチが別なら履歴も別）。各タスク内の粒度は §5 の基準に従う。
...
- job-graph: 並列タスク分解時、§2 に従いタスク単位のコミット計画を各エージェントの指示文へ含める
